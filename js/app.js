...
const showData = (books, searchKeyword) => {
  ...
  const newArr = books.docs.filter(
    (element) =>
      element.cover_i !== undefined &&
      element.author_name !== undefined &&
      element.publisher !== undefined &&
  ...
  if (newArr.length === 0) {
    ...
    newParagraph.innerHTML = `Total Search Result Found <b class="text-white">${totalResult}</b> books for <b class='searchKeyWord'>'${searchKeyword}'</b> showing <b class='text-white'>${newArr.length}</b> books`;
    result.style.background = "#1e272d";
    result.innerHTML = "";
